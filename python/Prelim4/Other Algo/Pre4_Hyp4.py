...
def solve(map: list[list[int]], n: int) -> list[list[tuple[int, int]]]:
    ...
    for station in stations:   
        for city in cities:
            ...
            station.available_cities.append((city, dist))
            station.weight += dist**2
        #station.available_cities.sort(key=lambda t: t[1])
        print("time:",time.process_time()*1000.0 - start_time)

    print("firstloop:", time.process_time()*1000.0 - start_time)
    ...
    for i in range(5):
        ...
        for station in stations:
            ...
            for t in station.available_cities:
                if t[0] not in cities:
                    station.available_cities.remove(t)
                    continue
            ...
            close_t[0].add_assign(station)
            cities.remove(close_t[0])
