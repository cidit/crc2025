...
void loop()
{
    ...
    // TODO: Hack tant qu'on doivent redémarrer le ALduino!! À retirer après
    if (!start_received)
    {
        if (ctrl.buttons.Up)
        {
    ...
    for (auto &pmotor : pmotors)
    {
        ...
        if (pmotor._name == "Poignet" || pmotor._name == "Bras Right")
        { //|| pmotor._name == "Bras Left"
            pmotor.update();
        }
    }
    pmotors[I_LB]._m.set_power_ratio(pmotors[I_RB]._pid_angle.getOutput()); // Update le moteur dont on utilise pas le PID
    ...
    if (ctrl.gachettes.Left || ctrl.gachettes.Right)
    {
        ...
        pmotors[I_RB].set_target_angle(angle_bras); // On utilise juste un PID pour le base du bras
